...
export default function DashboardPage() {
  ...
  // Update filters.search only when user submits
  ...
  useEffect(() => {
    ...
    if (loggedInUser.role.title?.toLowerCase() === "admin") {
  ...
  const handleAvatarUpdate = async (contactId: number, newAvatarUrl: string) => {
    ...
    if (selectedContact && selectedContact.id === contactId) {
      setSelectedContact(prev => prev ? { ...prev, avatar: newAvatarUrl } : null);
    }

    // Optionally reload all contacts to ensure consistency
    await loadContacts();
  };
