...
interface ContactImageModalProps {
  isOpen: boolean;
  onClose: () => void;
  contactId: number;
...
const ContactImageModal = ({ 
  ...
  contactName,
  onSuccess 
}: ContactImageModalProps) => {
  const [imageFile, setImageFile] = useState<File | null>(null);
  const [imagePreview, setImagePreview] = useState<string>(
    getQualifiedImageUrl(currentAvatar, false) || ""
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await apiService.put(`/contacts/${contactId}/image`, formData, {
        headers: {
      ...
      toast.error(error.response?.data?.message || "Failed to update contact image");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRemoveImage = () => {
    setImageFile(null);
    setImagePreview(getQualifiedImageUrl(currentAvatar, false) || "");
